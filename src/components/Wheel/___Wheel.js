...
const Wheel = () => {
  ...
  const wheelPrizesRef = useRef(null);

  const container = wheelPrizesRef.current;

  useEffect(() => {
    // this.$slider = this.wheelRef.current;
    // this.$Container = this.wheelPrizesRef.current;

    // this.$slides = Object.values(container.children);
    ...
    const slides = Object.values(container.children);

    setState({
      slides,
      rotateDirection: -1,
      rotationTimeLimit: getRandomInt(1500, 4000),
      slideAngle: 360 / slides.length,
      currentRotationAngle: 0,
      autoRotateIntervalId: false,
  ...
  const handleDragStart = (e) => {
    setState({
      ...state,
      readyToDrag: true,
      dragStartPoint: e.type === 'mousedown' ? e.pageX : e.originalEvent.touches[0].pageX
    });
  }
  ...
  const handlePlay = () => {
    state.rotateDirection < 0 ? startRotateRight() : startRotateLeft();
